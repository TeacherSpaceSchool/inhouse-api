        ...
        for(let i=0; i<stores.length; i++) {
            if(await WayItem.countDocuments({store: stores[i], arrivalDate: {$lt: date}, status: 'в пути'}).lean())
                await sendWebPush({
                    ...
                    users: await User.find({del: {$ne: true}, store: stores[i], role: {$in: ['admin', 'завсклад']}}).distinct('_id').lean()
                })
            ...
            if(await Installment.countDocuments({store: stores[i], arrivalDate: {$lt: date}, status: 'обработка'}).lean())
                await sendWebPush({
                    tag: 'Просрочена рассрочка',
                    title: 'Просрочена рассрочка',
                    message: 'Просрочена рассрочка',
                    users: await User.find({del: {$ne: true}, store: stores[i], role: {$in: ['admin', 'кассир']}}).distinct('_id').lean()
                })
            if(await Installment.countDocuments({store: stores[i], arrivalDate: date, status: 'обработка'}).lean())
                await sendWebPush({
            ...
            if(reservationManagers.length)
                await sendWebPush({
                    ...
                    users: await User.find({del: {$ne: true}, store: stores[i], $or:[{role: {$in: ['admin']}}, {_id: {$in: reservationManagers}}]}).distinct('_id').lean()
                })
