...
module.exports.saveFile = (stream, filename) => {
    return new Promise((resolve) => {
        ...
        let filepath = path.join(app.dirname, 'public', process.env.NODE_ENV==='test'?'test':'files', filename)
...
module.exports.saveImage = (stream, filename) => {
    return new Promise(async (resolve) => {
        ...
        let filepath = path.join(app.dirname, 'public', process.env.NODE_ENV==='test'?'test':'images', randomfilename)
        let fstream = fs.createWriteStream(filepath);
        stream.pipe(fstream)
        fstream.on('finish', async () => {
            ...
            if(image.bitmap.width>800||image.bitmap.height>800) {
                ...
                let filepathResize = path.join(app.dirname, 'public', process.env.NODE_ENV==='test'?'test':'images', randomfilename)
