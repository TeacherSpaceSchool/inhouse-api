        ...
        if(['admin', 'управляющий', 'менеджер', 'менеджер/завсклад', 'завсклад'].includes(user.role)) {
            ...
            for(let i = 0; i < res.length; i++) {
                ...
                for(let i1 = 0; i1 < res[i].bookings.length; i1++)
                    bookings = `${bookings?`${bookings}, `:''}${res[i].balance[i1].nameManager}|${res[i].balance[i1].manager}: ${res[i].balance[i1].amount}`
                worksheet.getRow(i+1).getCell(1).value = res[i]._id.toString()
                worksheet.getRow(i+1).getCell(2).value = res[i].status
                worksheet.getRow(i+1).getCell(3).value = `${res[i].item.name}|${res[i].item._id.toString()}`
                worksheet.getRow(i+1).getCell(4).value = `${res[i].store.name}|${res[i].store._id.toString()}`
                worksheet.getRow(i+1).getCell(5).value = res[i].amount
                worksheet.getRow(i+1).getCell(6).value = pdDDMMYYYY(res[i].arrivalDate)
                worksheet.getRow(i+1).getCell(7).value = bookings
            }
